...
import { HelpCircle, Clock, FastForward, CheckCircle2, XCircle, Play } from "lucide-react";
...
export default function GameScreen() {
  ...
  const {
    status, players, currentTurn, questions, currentQuestionIndex,
  ...
  // ── Round Transition Screen (Local Mode: between player 1 and player 2) ──
  ...
  return (
    <div className={`min-h-screen transition-colors duration-700 bg-gradient-to-br ${COLOR_MAP[currentPlayer.color]} p-4 md:p-8 flex flex-col text-white`}>
      ...
      {/* Header */}
      <header className="flex justify-between items-center mb-6 bg-black/20 backdrop-blur-md rounded-2xl p-4 shadow-lg gap-4">
        ...
        <div className="flex gap-4 shrink-0">
          {players.map(p => (
            <div key={p.id} className={`text-center transition-opacity ${p.id !== currentPlayer.id ? 'opacity-40' : ''}`}>
              <div className="text-xs opacity-70 mb-0.5">{p.name}</div>
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {currentQ.options.map((opt, i) => {
                ...
                if (showResult) {
                  if (isCorrect) {
                    cls = "bg-green-500/90 border-green-400 shadow-[0_0_20px_rgba(34,197,94,0.5)]";
                    icon = <CheckCircle2 className="w-6 h-6 ml-2 shrink-0" />;
                  } else if (isSelected) {
                    cls = "bg-red-500/80 border-red-400";
                    icon = <XCircle className="w-6 h-6 ml-2 shrink-0" />;
                  } else {
